...
public sealed class CoreBehaviorTests
{
    ...
    public void TranscriptCorrectionsApplyPhrasesAndWholeWords()
    {
        var corrections = new TranscriptCorrectionDictionary(
            [
                ...
                new TranscriptCorrection("p t t dictation", "PTT Dictation"),
                new TranscriptCorrection("c sharp", "C#")
            ]);

        var corrected = corrections.Apply("kuda is not a barracuda. p t t dictation likes c sharp");

        Assert.AreEqual("CUDA is not a barracuda. PTT Dictation likes C#", corrected);
    }
    ...
    [TestMethod]
    public async Task LongFinalPreviewExactlyMatchesTheTextSentToPaste()
    {
...
internal sealed class FakeDictationSession(string transcript) : IDictationSession
{
    ...
    {
        TranscriptUpdated?.Invoke(new TranscriptUpdate(TranscriptUpdateKind.Partial, text, unstableText));
    }
}
